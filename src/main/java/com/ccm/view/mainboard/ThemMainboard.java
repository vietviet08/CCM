		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					setDefaultIDMainboard(MainBoardDAO.getInstance().selectAll());
	...
	private void addMainboard() {
		...
		if (insert.length() > 0 && url.length() > 0)
			...
			if (insert.equals("") && url.equals("")) {
				int check = MainBoardDAO.getInstance().insertNotIMG(mb);
				insert = Checked.checkedAdd(check, insert);
			} else {
				if (url.equals("")) {
					int check = MainBoardDAO.getInstance().insert(mb);
					insert = Checked.checkedAdd(check, insert);
				} else if (insert.equals("")) {
					int check = MainBoardDAO.getInstance().insertIMGURL(mb, url);
					insert = Checked.checkedAdd(check, insert);
				}
			}
			MainboardForm.loadDataToTable(MainBoardDAO.getInstance().selectAll());
			closeFram();
		}
	}
